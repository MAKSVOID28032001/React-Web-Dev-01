...
const Filter = ({ OnFilterChange, ExpenseData, years }) => {
    ...
    useEffect(() => {
        ...
        if (SelectedYear === "All") {
            totalAmountSelectedYear = ExpenseData.reduce((total, expense) => total + parseInt(expense.eprice), 0);
        } else {
            totalAmountSelectedYear = ExpenseData
                .filter(expense => new Date(expense.edate).getFullYear().toString() === SelectedYear)
        ...
        const totalAmountAllYears = ExpenseData.reduce((total, expense) => total + parseInt(expense.eprice), 0);
        ...
        <>
            <div className="Expense-Filter">
                ...
                <div className="Total-Am-Box TAB1">
                    <p className="TM">Total Amount for {SelectedYear === "All" ? "All Years" : SelectedYear} : {TotalAmount}</p>
                    {SelectedYear !== "All" && (
                        <p className="TM">Total Amount for All Years: {TotalAmountAllYears}</p>
                    )}
                </div>
            </div>
        </>
