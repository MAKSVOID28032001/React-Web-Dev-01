...
const Form = () => {
    ...
    const HandleDelete = (expenseId) => {
        const updatedData = LocalStorageData.filter((item) => item.eid !== expenseId);
    ...
    //Edit Logic
    const [ExpenseObj, SetExpenseObj] = useState({ eid: "", ename: "", eabout: "", eprice: "", edate: "" });
    ...
    const SubmitExpenseHandler = (event) => {
        ...
        if (!ExpenseObj.ename || !ExpenseObj.eabout || !ExpenseObj.eprice || !ExpenseObj.edate) {
        ...
        Array.isArray(LocalStorageData) && LocalStorageData.length > 0 ?
            (FilteredYear === "All" ? LocalStorageData :
                LocalStorageData.filter(
                    (expense) => new Date(expense.edate).getFullYear() === parseInt(FilteredYear)
                )) :
            [];
    // Calculate Total Amount for the selected year and all years For Chart
    const totalAmountSelectedYear = FilteredYear === "All"
        ? LocalStorageData.reduce((total, expense) => total + parseInt(expense.eprice), 0)
        : FilteredExpense.reduce((total, expense) => total + parseInt(expense.eprice), 0);

    const totalAmountAllYears = FilteredExpense.reduce((total, expense) => total + parseInt(expense.eprice), 0);
    ...
    return (
        <>
                                ...
                                <div className="Box1">
                                    <div className="B1">
                                        <div className="Inside-Box">
                                            ...
                                            <input type="text" ref={name} className="IP" name="ename" onChange={SendExpense} value={ExpenseObj.ename} placeholder="Expense Name" required />
                                        </div>
                                        <div className="Inside-Box">
                                            <label className="LBL">Expense About:</label>
                                            <input type="text" ref={about} className="IP" name="eabout" onChange={SendExpense} value={ExpenseObj.eabout} placeholder="Expense About" required />
                                        </div>
                                    </div>
                                    <div className="B1">
                                        <div className="Inside-Box">
                                            <label className="LBL">Expense Price:</label>
                                            <input type="number" ref={price} className="IP" name="eprice" onChange={SendExpense} value={ExpenseObj.eprice} placeholder="Expense Price" required />
                                        </div>
                                        <div className="Inside-Box">
                                            <label className="LBL">Expense Date:</label>
                                            <input type="date" ref={date} className="IP" name="edate" onChange={SendExpense} value={ExpenseObj.edate} placeholder="Expense Date" max={`${CurrentYear}-12-31`} min={`${CurrentYear - 3}-01-01`} required />
                                        </div>
                                    </div>
                                </div>
                                <div className="Box2">
            ...
            <>
                <ExpenseChart expense={FilteredExpense} totalAmountAllYears={totalAmountAllYears} totalAmount={totalAmountSelectedYear} />
            </>
            <>
                <Filter
                ...
                {
                    FilteredYear === "All" ? (
                        <List value={LocalStorageData} onDelete={HandleDelete} onEdit={HandleEdit} year={CurrentYear} />
                    ) : (
                        <FilterYear
                            year={FilteredYear}
